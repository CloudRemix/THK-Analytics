...
class LoginHelper extends BaseHelper {
	...
	public function memory() {
		$memory = false;
		if( !$this->checkPost() ) {
			if( isset( $_COOKIE[Config::COOKIE_MEMORY] ) && $_COOKIE[Config::COOKIE_MEMORY] == 1 ) $memory = true;
		}
		else {
			if( $this->request->get('memory') === Config::ON ) $memory = true;
		}
		echo $memory ? 'checked="checked"' : '';
	}
}
?>
